...
import {
  ...
  Download, Radio, Clock, RefreshCw, ChevronRight, Hash,
  FileSpreadsheet, ShieldCheck, UserCheck, User, Play, Square,
} from "lucide-react";
import { SUBJECTS, subjectById, subjectLabel, TONE, winIndex, fmtTime, fmtDay, compressImage } from "./data";
import {
  ...
  exportSessionXlsx, exportSummaryXlsx,
} from "./supabase";
...
function SubjectDetail({ subjectId, onBack, onStartLive }) {
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {active ? (
          <button onClick={() => onStartLive(active)} className="w-full bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl p-4 flex items-center gap-3 transition">
            <Radio size={22} /><div className="text-left flex-1"><div className="font-bold">กลับเข้าคาบที่กำลังเปิด</div><div className="text-xs text-emerald-100">ครั้งที่ {active.no} · เช็คแล้ว {active.present}/{subject.roster.length}</div></div><ChevronRight />
          </button>
        ) : (
            ...
            {sessions.map((s) => (
              <div key={s.id} className="bg-white rounded-xl border border-slate-200 p-3 flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[11px] text-slate-400">มาเรียน {s.present}/{subject.roster.length} คน</div>
                </div>
  ...
  return (
    <>
      ...
      <div className="bg-white border-b border-slate-200 px-4 py-3 flex items-center justify-around text-center">
        ...
        <div><div className="text-2xl font-bold text-slate-300">{subject.roster.length - presentCount}</div><div className="text-[11px] text-slate-400">ยังไม่เช็ค</div></div>
        <div className="h-8 w-px bg-slate-200" />
        <div><div className="text-2xl font-bold text-slate-700">{subject.roster.length}</div><div className="text-[11px] text-slate-400">ทั้งหมด</div></div>
      </div>
      <div className="flex-1 overflow-y-auto p-3 space-y-1.5">
        {subject.roster.map((stu) => {
      ...
      {viewStu && checkins[viewStu] && (
        <StudentDetailModal subject={subject} session={session} checkin={checkins[viewStu]} onClose={() => setViewStu(null)}
...
function StudentDetailModal({ subject, session, checkin, onClose, onSaved }) {
  const stu = subject.roster.find((s) => s.id === checkin.studentId);
...
function StudentForm({ subject, session, onBack, onDone }) {
  ...
  const onSid = (v) => { setSid(v); setErr(""); setMatched(subject.roster.find((s) => s.id === v.trim()) || null); };
...
export default function App() {
  ...
  switch (route.v) {
    ...
    case "tSubject": content = <SubjectDetail subjectId={route.subjectId} onBack={() => go("tHome")} onStartLive={(sess) => go("tLive", { session: sess })} />; break;
